program prog_f
   ...
   !
   ! Goal, illustrate the use of iso_c_binding, focus on:
   !    Passing referance and value of scalar and pointer variables
   !
  ...
  interface
     ...
     subroutine c_sub1(varp) bind(c)
        ...
        ! Reference of scalar argument is passed to C function (must be a int pointer in the c function)
        integer(c_int)        :: varp
     end subroutine c_sub1
     
     subroutine c_sub2(varv) bind(c)
        use iso_c_binding, only : c_int
        ! Value     of scalar argument is passed to C function (must be a int         in the c function)
        integer(c_int), value :: varv
     end subroutine c_sub2

     subroutine c_sub3(varpp) bind(c)
       use iso_c_binding, only : c_ptr
       ! Reference of pointer argument is passed to C function (must be a pointer to a pointer in the c function)       
       type(c_ptr)        :: varpp
     end subroutine c_sub3

     subroutine c_sub4(varpv) bind(c)
       use iso_c_binding, only : c_ptr
       ! Value     of pointer argument is passed to C function (must be a pointer              in the c function)
